...
btnLogin.addEventListener('click', async () => {
    ...
    try {
        // SESSION فقط داخل دالة الكاشير — تنتهي عند إغلاق التبويب
        await auth.setPersistence(firebase.auth.Auth.Persistence.SESSION);
        await auth.signInWithEmailAndPassword(user + "@usr7n.com", pass);
    } catch {
...
// ========= إجبار الكاشير على تسجيل الدخول في كل زيارة =========
// signOut عند كل تحميل للصفحة بغض النظر عن أي جلسة محفوظة
auth.signOut().catch(e => console.warn("signOut on load:", e));
