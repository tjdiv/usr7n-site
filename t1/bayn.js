...
// تنظيف المنتجات المكررة بالاسم تلقائياً
setTimeout(async () => {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            if (seenNames.has(name)) {
                doc.ref.delete(); // يحذف النسخة الأحدث لأننا نرتب تصاعدياً
                deletedCount++;
                console.log("تم حذف نسخة مكررة من: " + name);
            } else {
                seenNames.add(name);
            }
        });
        
        // حذف المنتجات غير المرغوب بها (الطلب السابق والجديد)
        const unwantedSnapshot = await db.collection("products").where("name", "in", ["قهوه", "القهوه", "ميني", "كيك"]).get();
        unwantedSnapshot.forEach(doc => doc.ref.delete());

        if (deletedCount > 0) {
            console.log(`تم تنظيف ${deletedCount} منتجات مكررة بنجاح.`);
        }
    } catch (e) {
        console.error("خطأ في تنظيف التكرار: ", e);
    }
}, 2000);
