...
window.onunhandledrejection = function(event) {
    logErrorToFirebase({ type: "Dashboard Unhandled Promise", message: event.reason ? event.reason.toString() : "Unknown Promise Error" });
};
...
function listenToDeliveryStatus() {
    ...
    db.collection("settings").doc("delivery").onSnapshot((doc) => {
        if (doc.exists) {
            ...
            db.collection("settings").doc("delivery").set({ isOpen: true }).catch(e => console.error("Error creating delivery setting:", e));
        }
    }, (error) => {
        console.error("Firebase Delivery Status Error: ", error);
    });
...
const staticProducts = [
    ...
    { name: "كيكة تشوكليت", price: 24, imageUrl: "1/O/O/8.jpeg", isBuiltIn: true },
    { name: "اسبريسو", price: 10, imageUrl: "1/O/O/1.jpeg", isBuiltIn: true },
    { name: "كورتادو", price: 14, imageUrl: "1/O/O/2.jpeg", isBuiltIn: true },
    { name: "لاتيه", price: 14, imageUrl: "1/O/O/3.jpeg", isBuiltIn: true },
    { name: "تشيزبين", price: 18, imageUrl: "1/O/O/4.jpeg", isBuiltIn: true },
    ...
    { name: "قهوة اليوم حار", price: 16, imageUrl: "1/O/O/9.jpeg", isBuiltIn: true },
    { name: "تشيز كيك مانجو", price: 24, imageUrl: "1/O/O/11.jpeg", isBuiltIn: true },
    { name: "ميني تارت", price: 17, imageUrl: "1/O/O/13.jpeg", isBuiltIn: true },
...
function listenToOrders() {
    db.collection("orders").orderBy("createdAt", "desc")
        .onSnapshot((snapshot) => {
            ...
            console.error("Firebase Orders Admin Error: ", error);
            const list = document.getElementById('dashboard-orders-list');
            if (list) list.innerHTML = '<div style="color:red; padding:20px;">حدث خطأ في جلب الطلبات.</div>';
        });
}
        ...
        localProducts.forEach(p => {
            ...
            if (editPricesList) {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <button class="btn-admin edit" onclick="editProductPriceRow('${id}', '${p.name.replace(/'/g, "\\'")}', ${p.price})" style="background:var(--navy-deep); color:white; border:none; padding:6px 12px; border-radius:6px; cursor:pointer;">تعديل الاسم والسعر</button>
                    </td>
                `;
...
window.toggleRewardStatus = async (id, newStatus) => {
    ...
    try {
        await db.collection("products").doc(id).update({ isReward: newStatus, pointsPrice: pointsPrice });
...
window.editProductPriceRow = (id, name, price) => {
    ...
    actionsCell.innerHTML = `
        <button class="btn-admin save" onclick="saveProductPriceRow('${id}')" style="background:#2ecc71; color:white; border:none; padding:6px 12px; border-radius:6px; cursor:pointer; margin-left:5px;">حفظ</button>
        <button class="btn-admin cancel" onclick="loadAdminProducts()" style="background:#95a5a6; color:white; border:none; padding:6px 12px; border-radius:6px; cursor:pointer;">إلغاء</button>
    `;
};

window.saveProductPriceRow = async (id) => {
    const newName = document.getElementById(`inp-name-${id}`).value.trim();
    const newPrice = parseFloat(document.getElementById(`inp-price-${id}`).value);
...
// تنظيف نهائي وتحديث القائمة
if (!localStorage.getItem('menu_hard_reset_v1')) {
    setTimeout(async () => {
        try {
            const snapshot = await db.collection("products").get();
            const allowedNames = staticProducts.map(p => p.name);
            let deletedCount = 0;
            snapshot.forEach(doc => {
                if (!allowedNames.includes(doc.data().name)) {
                    doc.ref.delete();
                    deletedCount++;
                }
            });
            localStorage.setItem('menu_hard_reset_v1', 'true');
            if (deletedCount > 0) {
                console.log(`تم إعادة ضبط المنيو وحذف ${deletedCount} منتجات قديمة.`);
                setTimeout(() => window.location.reload(), 1500); // تحديث الصفحة لتظهر القائمة النظيفة
            }
